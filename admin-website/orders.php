...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $actionResponse=purchase_site_request('/api/admin-orders.php','POST',['order'=>(string)($_POST['order']??''),'action'=>$action]);
        if($action==='approve'){
            $licensePdo=database();ensure_license_management_schema($licensePdo);$cursor=0;
            for($page=0;$page<50;$page++){
                $licensesResponse=purchase_site_request('/api/admin-licenses.php?cursor='.$cursor);
                sync_purchase_licenses($licensePdo,is_array($licensesResponse['licenses']??null)?$licensesResponse['licenses']:[],'purchase-approval-sync');
                $next=$licensesResponse['nextCursor']??null;
                if(!is_int($next)&&!is_numeric($next))break;
                $next=(int)$next;if($next<=$cursor)throw new RuntimeException('The Buy website returned an invalid license cursor.');$cursor=$next;
            }
        }
        $_SESSION['order_flash'] = (string)($actionResponse['message']??($action === 'retry_email' ? 'The customer email was sent.' : 'The order was approved.'));
...
<aside class="sidebar"><nav><a href="/"><span aria-hidden="true">▥</span>Dashboard</a><a href="/customers.php"><span aria-hidden="true">◎</span>Customers</a><a href="/#installations"><span aria-hidden="true">□</span>Installations</a><a href="/licenses.php"><span aria-hidden="true">◇</span>License Manager</a><a class="active" href="/orders.php"><span aria-hidden="true">▤</span>Purchase Orders</a><a href="/pricing.php"><span aria-hidden="true">$</span>Purchase Pricing</a><a href="/communications.php"><span aria-hidden="true">✉</span>Communications</a><a href="/dev-support.php"><span aria-hidden="true">⌁</span>Dev Support</a></nav><p>Only verified PayPal payments can be approved.</p></aside>
...
<div class="order-workspace"><section class="order-list" aria-label="Orders"><?php if(!$orders):?><div class="empty-orders"><span>✓</span><h2>No orders in this queue</h2><p>New matching orders will appear here automatically.</p></div><?php else:?><?php foreach($orders as $order):$renewal=(string)($order['order_type']??'LICENSE')==='MAINTENANCE';?><a class="order-row <?=($selected['public_id']??'')===$order['public_id']?'selected':''?>" href="?status=<?=urlencode($status)?>&amp;order=<?=urlencode($order['public_id'])?>"><div><strong><?=e($order['customer_name'])?></strong><span><?=e(($order['license_tier']??'Pro').' '.($renewal?'maintenance renewal':'License').' · '.$order['email'])?></span></div><div><strong>$<?=e($order['amount'])?> <?=e($order['currency'])?></strong><span><?=e((string)($order['paid_at']??$order['created_at']))?> UTC</span></div><em><?=e($statusLabel($order['status']))?></em></a><?php endforeach;?><?php endif;?></section>
<aside class="order-detail"><?php if($selected):$renewal=(string)($selected['order_type']??'LICENSE')==='MAINTENANCE';?><div class="detail-heading"><span class="verified">✓ Verified payment</span><h2><?=e($selected['customer_name'])?></h2><a href="mailto:<?=e($selected['email'])?>"><?=e($selected['email'])?></a></div><dl><div><dt>Order</dt><dd><?=e($renewal?'Annual maintenance renewal':'Permanent license')?></dd></div><div><dt>License level</dt><dd><?=e((string)($selected['license_tier']??'Pro'))?></dd></div><div><dt>Amount</dt><dd>$<?=e($selected['amount'])?> <?=e($selected['currency'])?></dd></div><div><dt>PayPal order</dt><dd><?=e($selected['paypal_order_id'])?></dd></div><div><dt>Capture ID</dt><dd><?=e((string)($selected['paypal_capture_id']??'—'))?></dd></div><div><dt>Paid at</dt><dd><?=e((string)($selected['paid_at']??'—'))?> UTC</dd></div><?php if($renewal):?><div><dt>License ID</dt><dd><?=e((string)$selected['renewal_license_id'])?></dd></div><div><dt>Previous coverage</dt><dd><?=e((string)($selected['maintenance_previous_expires_at']??'—'))?></dd></div><?php if(!empty($selected['maintenance_new_expires_at'])):?><div><dt>Renewed through</dt><dd><?=e((string)$selected['maintenance_new_expires_at'])?></dd></div><?php endif;?><?php elseif(!empty($selected['license_id'])):?><div><dt>License ID</dt><dd><?=e($selected['license_id'])?></dd></div><?php endif;?></dl>
<?php if($selected['status']==='PAID_AWAITING_APPROVAL'):?><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="order" value="<?=e($selected['public_id'])?>"><input type="hidden" name="action" value="approve"><button class="approve-order">Approve &amp; email <?= $renewal?'renewal':'activation key' ?></button></form><?php elseif(in_array($selected['status'],['APPROVED','EMAIL_FAILED'],true)):?><div class="email-failure"><?=e((string)($selected['last_error']??'The order is approved but its delivery has not been confirmed.'))?></div><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="order" value="<?=e($selected['public_id'])?>"><input type="hidden" name="action" value="retry_email"><button class="approve-order">Retry customer email</button></form><?php else:?><div class="delivered">✓ <?= $renewal?'Renewal confirmation':'Activation key' ?> emailed <?=e((string)($selected['emailed_at']??''))?> UTC</div><?php endif;?><p class="order-safety"><?= $renewal?'Coverage is extended only after PayPal capture verification and your confirmation. Renewal never changes or expires the permanent license.':'A key is generated only after the server verifies a completed PayPal capture. It enters License Manager after delivery succeeds.' ?></p><?php else:?><div class="no-selection">Select an order to view its details.</div><?php endif;?></aside></div></main></div></body></html>
