...
<div class="order-workspace"><section class="order-list" aria-label="Orders"><?php if(!$orders):?><div class="empty-orders"><span>✓</span><h2>No orders in this queue</h2><p>New matching orders will appear here automatically.</p></div><?php else:?><?php foreach($orders as $order):?><a class="order-row <?=($selected['public_id']??'')===$order['public_id']?'selected':''?>" href="?status=<?=urlencode($status)?>&amp;order=<?=urlencode($order['public_id'])?>"><div><strong><?=e($order['customer_name'])?></strong><span><?=e(($order['license_tier']??'Pro').' License · '.$order['email'])?></span></div><div><strong>$<?=e($order['amount'])?> <?=e($order['currency'])?></strong><span><?=e((string)($order['paid_at']??$order['created_at']))?> UTC</span></div><em><?=e($statusLabel($order['status']))?></em></a><?php endforeach;?><?php endif;?></section>
<aside class="order-detail"><?php if($selected):?><div class="detail-heading"><span class="verified">✓ Verified payment</span><h2><?=e($selected['customer_name'])?></h2><a href="mailto:<?=e($selected['email'])?>"><?=e($selected['email'])?></a></div><dl><div><dt>License</dt><dd><?=e((string)($selected['license_tier']??'Pro'))?> License</dd></div><div><dt>Amount</dt><dd>$<?=e($selected['amount'])?> <?=e($selected['currency'])?></dd></div><div><dt>PayPal order</dt><dd><?=e($selected['paypal_order_id'])?></dd></div><div><dt>Capture ID</dt><dd><?=e((string)($selected['paypal_capture_id']??'—'))?></dd></div><div><dt>Paid at</dt><dd><?=e((string)($selected['paid_at']??'—'))?> UTC</dd></div><?php if(!empty($selected['license_id'])):?><div><dt>License ID</dt><dd><?=e($selected['license_id'])?></dd></div><?php endif;?></dl>
<?php if($selected['status']==='PAID_AWAITING_APPROVAL'):?><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="order" value="<?=e($selected['public_id'])?>"><input type="hidden" name="action" value="approve"><button class="approve-order">Approve &amp; email activation key</button></form><?php elseif(in_array($selected['status'],['APPROVED','EMAIL_FAILED'],true)):?><div class="email-failure"><?=e((string)($selected['last_error']??'The key is approved but its delivery has not been confirmed.'))?></div><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="order" value="<?=e($selected['public_id'])?>"><input type="hidden" name="action" value="retry_email"><button class="approve-order">Retry activation email</button></form><?php else:?><div class="delivered">✓ Activation key emailed <?=e((string)($selected['emailed_at']??''))?> UTC</div><?php endif;?><p class="order-safety">A key is generated only after the server verifies a completed PayPal capture. It enters License Manager after delivery succeeds.</p><?php else:?><div class="no-selection">Select an order to view its details.</div><?php endif;?></aside></div></main></div></body></html>
