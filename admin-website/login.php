...
?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Admin Portal sign in | POS Printer Emulator</title><link rel="icon" type="image/png" href="assets/favicon.png"><link rel="stylesheet" href="assets/admin.css?v=20260715-3"></head><body class="login-page"><main class="login-panel"><img src="assets/icon-web.png" alt=""><h1>Admin Portal</h1><p>Sign in to the protected POS Printer Emulator administration area.</p><?php if ($error !== ''): ?><div class="login-error" role="alert"><?= e($error) ?></div><?php endif; ?><form method="post"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><label>Username<input name="username" autocomplete="username" required autofocus></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button type="submit">Sign in</button></form><small>Dashboard, License Manager, Purchase Pricing, and Dev Support access is restricted.</small></main></body></html>
