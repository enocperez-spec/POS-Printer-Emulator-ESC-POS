...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $issued = issue_activation_key($customerName, $emailAddress);
        $insert = database()->prepare(
            'INSERT INTO issued_licenses
                (license_id, customer_name, email_address, activation_key, issued_at, created_by)
             VALUES (:license_id, :customer_name, :email_address, :activation_key, :issued_at, :created_by)'
        );
...
$licenses = database()->query(
    "SELECT l.license_id, l.customer_name, l.email_address, l.activation_key, l.issued_at, l.revoked_at,
            EXISTS(
                SELECT 1 FROM installations i
                WHERE i.license_id = l.license_id AND i.license_mode = 'Full'
            ) AS activated
...
<aside class="sidebar"><nav><a href="/"><span aria-hidden="true">▥</span>Dashboard</a><a href="/#installations"><span aria-hidden="true">□</span>Installations</a><a class="active" href="/licenses.php"><span aria-hidden="true">◇</span>License Manager</a><a href="/orders.php"><span aria-hidden="true">▤</span>Purchase Orders</a><a href="/pricing.php"><span aria-hidden="true">$</span>Purchase Pricing</a><a href="https://posprinteremulator.com/privacy.html"><span aria-hidden="true">⚙</span>Settings</a></nav><p>The private signing key stays protected on the server.</p></aside>
<main class="license-main"><div class="page-heading"><div><h1>License Manager</h1><p>Generate signed Full Version keys and track issued licenses.</p></div></div>
<section class="generator-panel">
<div class="generator-form"><span class="eyebrow">Full Version</span><h2>Generate customer key</h2><p>Enter the customer details exactly as they appear in the desktop application.</p>
<?php if ($error !== ''): ?><div class="form-error" role="alert"><?= e($error) ?></div><?php endif; ?>
<form method="post" autocomplete="off"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><label>Customer or company name<input name="customer_name" maxlength="160" value="<?= e($customerName) ?>" required></label><label>Email address<input type="email" name="email_address" maxlength="254" value="<?= e($emailAddress) ?>" required></label><button class="primary-button" type="submit"><span aria-hidden="true">＋</span> Generate activation key</button></form></div>
