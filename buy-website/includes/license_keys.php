...
function issue_activation_key(string $customerName, string $emailAddress): array
{
    ...
    $payload = chr(1) . $guidBytes . pack('N2', 0, $timestamp) . $hash($customerName) . $hash($emailAddress);
    ...
    return ['license_id'=>$licenseId, 'activation_key'=>'PPE1-'.rtrim(strtr(base64_encode($payload.$signature),'+/','-_'),'=')];
}
