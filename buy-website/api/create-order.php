...
try {
    ...
    $offer=license_offer(); $amount=$offer['price']; $currency=$offer['currency'];
    ...
    $publicId=random_public_id(); $created=paypal_request('POST','/v2/checkout/orders',[
      'intent'=>'CAPTURE','purchase_units'=>[['reference_id'=>$publicId,'description'=>(string)config('license.product_name'),'amount'=>['currency_code'=>$currency,'value'=>$amount]]],
    ...
    $q=db()->prepare('INSERT INTO orders(public_id,customer_name,email,paypal_order_id,amount,currency,status,created_at) VALUES(?,?,?,?,?,?,?,?)');
    $q->execute([$publicId,$name,$email,$paypalOrderId,$amount,$currency,'CREATED',now_utc()]); $id=(int)db()->lastInsertId(); audit($id,'ORDER_CREATED',$paypalOrderId);
