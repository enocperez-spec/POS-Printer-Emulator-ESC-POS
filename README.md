...
1. Download `POSPrinterEmulatorSetup-0.3.02-win-x64.exe` from the repository's Releases page.
...
Output: `artifacts\installer\POSPrinterEmulatorSetup-0.3.02-win-x64.exe`
...
```console
gh auth login
gh release create v0.3.02 artifacts/installer/POSPrinterEmulatorSetup-0.3.02-win-x64.exe --title "POS Printer Emulator 0.3.02" --notes "Updated the built-in Test Receipt customer and check details."
```
...
```console
POSPrinterEmulatorSetup-0.3.02-win-x64.exe /VERYSILENT /CustomerName="Company Name" /CustomerEmail="customer@example.com"
```
