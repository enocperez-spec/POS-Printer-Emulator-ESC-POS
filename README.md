...
- The v0.3.50 release creates a privacy-reviewed, logo-branded Advanced Diagnostics PDF for Enterprise support and developer investigation.
...
> **Release status:** v0.3.50 Advanced Diagnostics PDF Report is the current public release, released July 24, 2026.
...
1. Download `POSPrinterEmulatorSetup-0.3.50-win-x64.exe` from the repository's Releases page.
...
Output for the current release: `artifacts\installer\POSPrinterEmulatorSetup-0.3.50-win-x64.exe`
...
The installer reads its product artwork from `assets\branding`. The square `pos-printer-emulator-icon.png` is used for the small wizard header, while the purpose-built `pos-printer-emulator-installer-banner.png` is used for the tall welcome/completion panel so the logo is never stretched. Keep the banner at the Inno Setup `164:314` aspect ratio (the supplied high-resolution asset is `656x1256`). Replace `pos-printer-emulator.ico` to update the Setup executable, application shortcuts, and uninstall entry. Keep the filenames unchanged. The packaging command verifies all branding files, required installer directives, and the banner aspect ratio before compiling.
