...
1. Download `POSPrinterEmulatorSetup-0.3.01-win-x64.exe` from the repository's Releases page.
...
Output: `artifacts\installer\POSPrinterEmulatorSetup-0.3.01-win-x64.exe`
...
```console
gh auth login
gh release create v0.3.01 artifacts/installer/POSPrinterEmulatorSetup-0.3.01-win-x64.exe --title "POS Printer Emulator 0.3.01" --notes "Collapsible workspace panels, job deletion, and improved Epson command parsing."
```
...
```console
POSPrinterEmulatorSetup-0.3.01-win-x64.exe /VERYSILENT /CustomerName="Company Name" /CustomerEmail="customer@example.com"
```
