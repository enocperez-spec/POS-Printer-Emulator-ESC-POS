...
header('Location: https://admin.posprinteremulator.com/', true, 301);
exit;
...
header("Content-Security-Policy: default-src 'self'; img-src 'self'; style-src 'self'; script-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");

$loginError = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    if (verify_admin_password(trim((string)($_POST['username'] ?? '')), (string)($_POST['password'] ?? ''))) {
...
<title>Owner sign in | POS Printer Emulator</title><link rel="icon" type="image/png" href="../assets/favicon.png"><link rel="stylesheet" href="dashboard.css?v=20260714-1"></head>
<body class="login-page"><main class="login-panel"><img src="../assets/icon-web.png" alt=""><h1>POS Printer Emulator</h1><p>Sign in to the License &amp; Usage dashboard.</p>
<?php if ($loginError !== ''): ?><div class="login-error" role="alert"><?= htmlspecialchars($loginError) ?></div><?php endif; ?>
<form method="post"><label>Username<input name="username" autocomplete="username" required autofocus></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button type="submit" name="login" value="1">Sign in</button></form>
<small>No receipt contents are collected.</small></main></body></html><?php exit; endif;
...
?><!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>License &amp; Usage | POS Printer Emulator</title><link rel="icon" type="image/png" href="../assets/favicon.png"><link rel="stylesheet" href="dashboard.css?v=20260714-1"><link rel="stylesheet" href="dashboard-overrides.css?v=20260714-1"></head>
<body><div class="app-shell"><header class="topbar"><a class="brand" href="index.php"><img src="../assets/icon-web.png" alt=""><span>POS Printer Emulator</span></a><form method="post" class="logout-form"><span>Owner Account</span><input type="hidden" name="csrf" value="<?= htmlspecialchars((string)$_SESSION['csrf']) ?>"><button name="logout" value="1">Log out</button></form></header>
<aside class="sidebar"><nav><a class="active" href="#dashboard"><span aria-hidden="true">▥</span>Dashboard</a><a href="#installations"><span aria-hidden="true">□</span>Installations</a><a href="#installations" data-license-link><span aria-hidden="true">◇</span>Licenses</a><a href="#privacy"><span aria-hidden="true">⚙</span>Settings</a></nav><p id="privacy">No receipt contents<br>are collected.</p></aside>
...
<script id="usage-data" type="application/json"><?= json_encode($series, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?></script><script src="dashboard.js?v=20260714-1"></script></body></html>
