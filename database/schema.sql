...
VALUES
    ...
    ('v0.3.34', 'v0.3.34', 'Release', 'Encrypted configuration backup and restore', 'Released', 334, 'Protect and move emulator configuration without transferring license or machine identity.', 'Password-encrypted PPE backup packages; listeners, profiles, stored logos, printer states, preferences, and optional paid history; explicit exclusions; verified preview; license-limit preservation; Windows-protected safety snapshots; rollback-safe restore.', 'Customers need a recovery path before comparison suites and the guided updater add more reusable configuration.', 'Encrypted create, inspect, and restore pass with wrong-password and tamper rejection, automatic safety snapshots, rollback protection, all 151 tests, and rendered desktop/mobile verification.', '2026-07-21 00:00:00.000000'),
    ('v0.3.35', 'v0.3.35', 'Release', 'Receipt comparison and automated validation', 'Next', 335, 'Provide repeatable compatibility and regression testing.', 'Compare bytes, commands, text, warnings, and rendered output, with saved baselines, ignored dynamic fields, validation suites, and HTML, PDF, and JSON results.', 'The encrypted v0.3.34 backup foundation protects the profiles, listeners, and captures used by comparison suites.', 'Known-good captures pass, intentional changes fail precisely, and ignored dynamic fields avoid false failures.', NULL),
    ('v0.3.36', 'v0.3.36', 'Release', 'Guided update installation and restart', 'Planned', 336, 'Close the application safely before an update replaces installed files, then return the customer to the updated application.', 'Background installer download; checksum and signature verification; pre-update safety snapshot; Install and Restart, Install Later, and Cancel choices; active-job drain; listener and service shutdown; external updater process; file-lock wait; state preservation; minimal-prompt installation; automatic relaunch; success confirmation; logs; rollback-safe failure recovery; optional automatic downloads.', 'A controlled external updater eliminates self-update file locks without unexpected listener downtime or lost customer state.', 'Install and Restart completes without locked-file errors, relaunches the new version, preserves customer state and data, and leaves the current installation usable after cancellation or failure.', NULL),
...
UPDATE development_roadmap
SET version_label = 'v0.3.34', title = 'Encrypted configuration backup and restore', status = 'Released', priority_rank = 334,
    purpose = 'Protect and move emulator configuration without transferring license or machine identity.',
    planned_scope = 'Password-encrypted PPE backup packages; listeners, profiles, stored logos, printer states, preferences, and optional paid history; explicit exclusions; verified preview; license-limit preservation; Windows-protected safety snapshots; rollback-safe restore.',
    priority_reason = 'Customers need a recovery path before comparison suites and the guided updater add more reusable configuration.',
    completion_criteria = 'Encrypted create, inspect, and restore pass with wrong-password and tamper rejection, automatic safety snapshots, rollback protection, all 151 tests, and rendered desktop/mobile verification.',
    completed_at = COALESCE(completed_at, '2026-07-21 00:00:00.000000')
...
VALUES
    ('v0.3.35', 'v0.3.35', 'Release', 'Receipt comparison and automated validation', 'Next', 335,
     ...
     'The encrypted v0.3.34 backup foundation protects the profiles, listeners, and captures used by comparison suites.',
     'Known-good captures pass, intentional changes fail precisely, and ignored dynamic fields avoid false failures.')
